...
const NATIONS = [
  { code: "BR", name: "Brazil", flag: "\u{1F1E7}\u{1F1F7}", color: "#009C3B" },
  { code: "AR", name: "Argentina", flag: "\u{1F1E6}\u{1F1F7}", color: "#74ACDF" },
  { code: "FR", name: "France", flag: "\u{1F1EB}\u{1F1F7}", color: "#002395" },
  { code: "DE", name: "Germany", flag: "\u{1F1E9}\u{1F1EA}", color: "#000000" },
  { code: "ES", name: "Spain", flag: "\u{1F1EA}\u{1F1F8}", color: "#AA151B" },
  { code: "EN", name: "England", flag: "\u{1F3F4}\u{E0067}\u{E0062}\u{E0065}\u{E006E}\u{E0067}\u{E007F}", color: "#CF081F" },
  { code: "PT", name: "Portugal", flag: "\u{1F1F5}\u{1F1F9}", color: "#006600" },
  { code: "NG", name: "Nigeria", flag: "\u{1F1F3}\u{1F1EC}", color: "#008751" },
  { code: "JP", name: "Japan", flag: "\u{1F1EF}\u{1F1F5}", color: "#BC002D" },
  { code: "MX", name: "Mexico", flag: "\u{1F1F2}\u{1F1FD}", color: "#006847" },
  { code: "US", name: "USA", flag: "\u{1F1FA}\u{1F1F8}", color: "#B22234" },
  { code: "MA", name: "Morocco", flag: "\u{1F1F2}\u{1F1E6}", color: "#C1272D" }
];
...
function PitchLines() {
  return <svg viewBox="0 0 400 260" style={{ position: "absolute", inset: 0, width: "100%", height: "100%", opacity: 0.06, pointerEvents: "none" }}>
...
function HeroScreen({ onStart }) {
  ...
  return <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", position: "relative", overflow: "hidden", padding: "2rem 1rem", textAlign: "center" }}>
      <ParticleField />
    ...
    onMouseEnter={(e) => {
      ...
      e.target.style.boxShadow = "0 0 50px rgba(0,255,135,0.7)";
    }}
    onMouseLeave={(e) => {
      e.target.style.transform = "scale(1)";
      e.target.style.boxShadow = "0 0 30px rgba(0,255,135,0.4)";
    }}
  >
          Deploy Your Gaffer →
        </button>
    ...
    /* Stats row */
        ...
        <div style={{ display: "flex", gap: "2rem", justifyContent: "center", marginTop: "3rem", flexWrap: "wrap" }}>
          {[["32", "Nations"], ["\u221E", "AI Gaffers"], ["100%", "Onchain"]].map(([num, label]) => <div key={label} style={{ textAlign: "center" }}>
...
function DeployScreen({ onDeploy }) {
  ...
  return <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "2rem 1rem", position: "relative" }}>
      <PitchLines />
    ...
    onMouseEnter={(e) => {
      if (selectedNation?.code !== n.code) e.currentTarget.style.borderColor = "rgba(0,255,135,0.4)";
    }}
    onMouseLeave={(e) => {
      if (selectedNation?.code !== n.code) e.currentTarget.style.borderColor = "rgba(255,255,255,0.08)";
    }}
  >
            ...
            <button onClick={() => selectedNation && setStep(2)} style={{ width: "100%", background: selectedNation ? "#00FF87" : "rgba(255,255,255,0.05)", color: selectedNation ? "#000" : "rgba(255,255,255,0.2)", border: "none", borderRadius: "4px", padding: "1rem", fontFamily: "'Anton', sans-serif", fontSize: "1rem", letterSpacing: "0.1em", textTransform: "uppercase", cursor: selectedNation ? "pointer" : "not-allowed", transition: "all 0.2s" }}>
              Continue →
            </button>
          </div>}
  ...
  />
    ...
    /* Tactic DNA preview */
            ...
            {name && <div style={{ background: "rgba(0,255,135,0.06)", border: "1px solid rgba(0,255,135,0.15)", borderRadius: "8px", padding: "1rem", marginBottom: "1.5rem", animation: "fadeIn 0.4s ease" }}>
                ...
                <div style={{ display: "flex", gap: "0.75rem", flexWrap: "wrap" }}>
                  {["Aggressive Press", "High Line", "Counter-Attack Ready"].map((t) => <span key={t} style={{ background: "rgba(0,255,135,0.1)", border: "1px solid rgba(0,255,135,0.2)", borderRadius: "100px", padding: "0.25rem 0.75rem", fontFamily: "'Space Mono', monospace", fontSize: "0.6rem", color: "#00FF87" }}>{t}</span>)}
                </div>
              </div>}

            <div style={{ display: "flex", gap: "0.75rem" }}>
              <button onClick={() => setStep(1)} style={{ background: "transparent", border: "1.5px solid rgba(255,255,255,0.1)", borderRadius: "4px", padding: "1rem", color: "rgba(255,255,255,0.4)", fontFamily: "'Space Mono', monospace", fontSize: "0.75rem", cursor: "pointer", letterSpacing: "0.05em" }}>← Back</button>
              <button
  ...
  >
            ...
            {deploying && <div style={{ marginTop: "1.5rem", animation: "fadeIn 0.3s ease" }}>
...
function WarRoom({ gaffer, onResult }) {
  ...
  const handleStake = () => {
    ...
    setTimeout(() => {
      setStaked(true);
      setTimeout(() => onResult({ gaffer, ...output }), 2e3);
    }, 2e3);
  };
  ...
  >
    ...
    /* Output */
        ...
        {output && !thinking && <div style={{ animation: "slideUp 0.5s cubic-bezier(.16,1,.3,1)" }}>
            {
    /* Tactic */
  }
    ...
    /* Trash talk */
            ...
            <div style={{ background: "rgba(255,215,0,0.06)", border: "1px solid rgba(255,215,0,0.2)", borderRadius: "12px", padding: "1.25rem", marginBottom: "1rem" }}>
              <div style={{ fontFamily: "'Space Mono', monospace", fontSize: "0.6rem", color: "#FFD700", letterSpacing: "0.15em", textTransform: "uppercase", marginBottom: "0.75rem" }}>🗣️ Gaffer's Pre-Match Statement</div>
              <div style={{ fontFamily: "'Barlow Condensed', sans-serif", fontSize: "1.15rem", color: "#fff", fontStyle: "italic", lineHeight: 1.5, minHeight: "2rem" }}>"{tauntDisplayed}<span style={{ animation: "blink 1s infinite" }}>|</span>"</div>
            </div>
    ...
    /* Stake */
            ...
            <div style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(255,255,255,0.08)", borderRadius: "12px", padding: "1.25rem", marginBottom: "1.5rem" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ fontFamily: "'Anton', sans-serif", fontSize: "1.8rem", color: "#fff" }}>{output.stakeAmt} <span style={{ fontSize: "1rem", color: "#00FF87" }}>OKB</span></div>
  ...
  >
              {staked ? "\u2713 Staked Onchain \u2014 Loading Results..." : staking ? "\u27F3 Broadcasting to X Layer..." : "Confirm Stake + Go Onchain \u26D3\uFE0F"}
            </button>
          </div>}
      </div>
    </div>;
...
function ResultScreen({ result }) {
  ...
  const score = won ? `${Math.floor(Math.random() * 3) + 1}-${Math.floor(Math.random() * 2)}` : `${Math.floor(Math.random() * 2)}-${Math.floor(Math.random() * 3) + 1}`;
  const txHash = "0x" + Array.from({ length: 16 }, () => Math.floor(Math.random() * 16).toString(16)).join("");
  const shareText = `\u{1F9E0}\u26BD My AI Gaffer "${result.gaffer.name}" just called it.

${result.gaffer.nation.flag} ${result.gaffer.nation.name} ${score} | Formation: ${result.tactic}

Receipts onchain on @XLayerOfficial \u26D3\uFE0F
  ...
  return <div style={{ minHeight: "100vh", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", padding: "2rem 1rem", position: "relative" }}>
    ...
    /* Result header */
        ...
        <div style={{ textAlign: "center", marginBottom: "2rem" }}>
          <div style={{ fontSize: "3rem", marginBottom: "0.5rem" }}>{won ? "\u{1F3C6}" : "\u{1F480}"}</div>
    ...
    /* Receipt card */
        ...
        <div style={{ background: "rgba(255,255,255,0.03)", border: `1px solid ${won ? "rgba(0,255,135,0.2)" : "rgba(255,68,68,0.2)"}`, borderRadius: "16px", padding: "1.5rem", marginBottom: "1.5rem" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "0.75rem", marginBottom: "1rem" }}>
            {[["Gaffer", result.gaffer.name], ["Nation", result.gaffer.nation.name], ["Stake", `${result.stakeAmt} OKB`], ["Return", won ? `${(parseFloat(result.stakeAmt) * 1.9).toFixed(1)} OKB` : "0 OKB"]].map(([label, value]) => <div key={label}>
  ...
  return <>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        ::-webkit-scrollbar { width: 4px; } ::-webkit-scrollbar-track { background: #070a0f; } ::-webkit-scrollbar-thumb { background: rgba(0,255,135,0.3); border-radius: 2px; }
      `}</style>

      {screen === "hero" && <HeroScreen onStart={() => setScreen("deploy")} />}
      {screen === "deploy" && <DeployScreen onDeploy={(g) => {
    setGaffer(g);
    setScreen("warroom");
  }} />}
      {screen === "warroom" && gaffer && <WarRoom gaffer={gaffer} onResult={(r) => {
    setResult(r);
    setScreen("result");
  }} />}
