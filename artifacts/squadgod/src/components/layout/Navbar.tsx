...
import { Activity, Users, Shield, Crosshair, ChevronRight } from "lucide-react";

export function Navbar() {
  const [location] = useLocation();

  return (
    <nav className="border-b border-primary/20 bg-background/95 backdrop-blur z-50 sticky top-0">
...
function NavLink({ href, active, children }: { href: string; active: boolean; children: React.ReactNode }) {
  return (
    <Link 
      href={href} 
      className={`px-3 py-2 text-sm font-mono tracking-wider transition-all duration-200 border-b-2 ${
        active 
          ? "border-primary text-primary glow-text bg-primary/5" 
