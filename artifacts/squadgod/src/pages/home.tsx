...
export default function Home() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-in fade-in zoom-in duration-500">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <Card className="bg-card/50 border-primary/20 rounded-none cyber-grid lg:col-span-1">
          ...
          <CardContent className="p-0">
            <div className="divide-y divide-primary/10">
              {isTopPlayersLoading ? (
                <div className="p-6 text-center text-muted-foreground font-mono text-sm">FETCHING DATA...</div>
              ) : (
                topPlayers?.map(player => (
                  <Link key={player.id} href={`/players/${player.id}`} className="flex items-center justify-between p-4 hover:bg-primary/5 transition-colors group cursor-pointer">
        ...
        <Card className="bg-card/50 border-primary/20 rounded-none cyber-grid lg:col-span-1">
          ...
          <CardContent className="p-0">
             <div className="divide-y divide-primary/10">
              {isSquadsLoading ? (
                <div className="p-6 text-center text-muted-foreground font-mono text-sm">FETCHING SQUADS...</div>
              ) : squads?.length === 0 ? (
                <div className="p-6 text-center text-muted-foreground font-mono text-sm">NO UNITS FOUND</div>
              ) : (
        ...
        <Card className="bg-card/50 border-primary/20 rounded-none cyber-grid lg:col-span-1">
          ...
          <CardContent className="p-0">
             <div className="divide-y divide-primary/10">
              {isMatchesLoading ? (
                <div className="p-6 text-center text-muted-foreground font-mono text-sm">PROCESSING PREDICTIONS...</div>
              ) : matches?.length === 0 ? (
                <div className="p-6 text-center text-muted-foreground font-mono text-sm">NO PREDICTIONS FOUND</div>
              ) : (
                matches?.map(match => (
                  <div key={match.id} className="p-4 hover:bg-primary/5 transition-colors">
...
function StatCard({ title, value, icon, glow = "primary" }: { title: string; value: React.ReactNode; icon: React.ReactNode; glow?: "primary" | "secondary" }) {
  return (
    <Card className="rounded-none border-primary/20 bg-card/50 backdrop-blur">
