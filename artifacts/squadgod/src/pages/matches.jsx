...
function Matches() {
  ...
  return <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8 animate-in fade-in duration-500">
      <div>
  ...
  >
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          {isLoading ? <div className="space-y-4">
              {[1, 2, 3].map((i) => <Card key={i} className="bg-card/50 border-primary/20 h-32 animate-pulse cyber-grid" />)}
            </div> : matches?.length === 0 ? <div className="text-center py-12 border border-primary/20 bg-card/30 cyber-grid">
              <p className="font-mono text-muted-foreground">NO_LOGS_FOUND</p>
            </div> : <div className="space-y-4">
              {matches?.map((match) => <Card key={match.id} className="bg-card/50 border-primary/20 rounded-none overflow-hidden">
