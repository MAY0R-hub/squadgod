            ...
            <div className="z-10 w-full h-full p-8 flex flex-col justify-between relative text-center">
              ...
              <div className="grid grid-cols-5 gap-4 h-full relative z-20 place-items-center">
                {POSITIONS.map(pos => {
                  ...
                  return (
                    <div key={pos} className="w-16 h-16 border-2 border-primary/30 rounded-full flex items-center justify-center bg-card/80 backdrop-blur">
                      {member ? (
                        ...
                        <span className="text-muted-foreground/50 text-xs font-mono">{pos}</span>
                      )}
                    </div>
                  );
                })}
            ...
            <CardContent className="p-0 overflow-auto flex-1">
              <div className="divide-y divide-primary/10">
                {squad.players.length === 0 ? (
                  <div className="p-6 text-center text-muted-foreground font-mono text-sm">NO_ASSETS_ASSIGNED</div>
                ) : (
                  squad.players.map(member => (
                    <PlayerRow key={member.id} squadId={squad.id} member={member} />
                  ))
                )}
              </div>
            </CardContent>
...
function PlayerRow({ squadId, member }: { squadId: number, member: any }) {
  const queryClient = useQueryClient();
  ...
  return (
    <div className="p-4 hover:bg-primary/5 transition-colors flex items-center justify-between group">
      <div>
...
function EditSquadDialog({ squad }: { squad: any }) {
...
function AddPlayerDialog({ squadId, squadPlayers }: { squadId: number, squadPlayers: any[] }) {
  const [open, setOpen] = useState(false);
  const [slot, setSlot] = useState(POSITIONS[0]);
  const [search, setSearch] = useState("");
  const { data: players, isLoading } = useListPlayers({ search: search.length > 2 ? search : undefined });
      ...
      <DialogContent className="bg-card border-primary/50 rounded-none cyber-grid sm:max-w-[500px]">
        ...
        <div className="grid gap-4 py-4">
            ...
            <Select value={slot} onValueChange={setSlot}>
              ...
              <SelectContent className="rounded-none border-primary/50">
                ...
                {availableSlots.length === 0 && (
                  <SelectItem value="FULL" disabled>ALL_SLOTS_OCCUPIED</SelectItem>
                )}
              </SelectContent>
            </Select>
          </div>
          ...
          <div className="h-64 overflow-y-auto border border-primary/20 bg-background/50 divide-y divide-primary/10">
            {isLoading ? (
              <div className="p-4 text-center text-muted-foreground font-mono text-sm">SEARCHING...</div>
            ) : players?.length === 0 ? (
              <div className="p-4 text-center text-muted-foreground font-mono text-sm">NO_MATCHES</div>
            ) : (
              players?.map(player => {
                const isAlreadyInSquad = squadPlayers.some(sp => sp.playerId === player.id);
                return (
                  <div key={player.id} className="p-3 flex items-center justify-between hover:bg-primary/5 transition-colors">
                    <div>
...
function AnalysisDialog({ squadId }: { squadId: number }) {
  const [open, setOpen] = useState(false);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="bg-card border-secondary/50 rounded-none cyber-grid sm:max-w-[600px]">
        ...
        {isLoading || isFetching ? (
            ...
            <div className="bg-secondary/10 border border-secondary/30 p-4">
              ...
              <ul className="space-y-2">
                {analysis.suggestions.map((s, i) => (
                  <li key={i} className="text-sm font-mono text-secondary/80">
                    &gt; {s}
                  </li>
                ))}
              </ul>
          ...
          <div className="py-8 text-center font-mono text-destructive">ANALYSIS_FAILED</div>
        )}
      </DialogContent>
    </Dialog>
  );
